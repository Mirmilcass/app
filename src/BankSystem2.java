...
public class BankSystem2 {
	...
	*/
	...
	private ArrayList<Customer2> custs;

	BankSystem2() {

	}

	public BankSystem2(int size) {
		scan = new Scanner(System.in);
		//		custs = new Customer[size];
		custs = new ArrayList<Customer2>();
		/*
	...
	public void outInfo() {
		...
		if (cust_idx != 0) {
			for (int i = 0; i < cust_idx; i++) {
				if (wantId.equals(custs.get(i).getId())) {
					System.out.println("사용자의 아이디는 : "
							+ custs.get(i).getId() + "\n사용자 암호는 : "
							+ custs.get(i).getPw() + "\n사용자의 잔액은 "
							+ custs.get(i).getMoney() + "원입니다.\n"
							+ custs.get(i).getVip() + "입니다.\n");
					break;
				} else
	...
	public void outAllcustinfo() {
		...
		System.out.println("===============");
		for (int i = 0; i < cust_idx; i++) {
			if (custs.get(i).getId() != null) {
				System.out.println("사용자의 아이디는 : " + custs.get(i).getId()
						+ "\n사용자 암호는 : " + custs.get(i).getPw()
						+ "\n사용자의 잔액은 " + custs.get(i).getMoney()
						+ "원입니다.\n" + custs.get(i).getVip() + "입니다.\n");
			}
		}
	}
	...
	public void deposit() {
		...
		loop: while (true) {
			for (int i = 0; i < cust_idx; i++) {
				...
				String id = inputStr();
				if (id.equals(custs.get(i).getId())) {
					System.out.println("사용자 암호를 입력하세요.");
					String pw = inputStr();
					if (pw.equals(custs.get(i).getPw())) {
						System.out.println("사용자 인증 되었습니다.");
						System.out.println("원하시는 입금액을 입력하세요.");
						custs.get(i).setMoney(
								custs.get(i).getMoney() + inputInt());
						System.out.println("사용자의 잔액은 "
								+ custs.get(i).getMoney() + "입니다.");
						break loop;
					} else
			...
			for (int i = 0; i < cust_idx; i++) {
				...
				if (id.equals(custs.get(i).getId())) {
					...
					if (pw.equals(custs.get(i).getPw())) {
						...
						int money = inputInt();
						if (money < custs.get(i).getMoney()) {
							custs.get(i).setMoney(
									custs.get(i).getMoney()
											- (money + custs.get(i)
													.getTax()));
							System.out.println("사용자의 잔액은 "
									+ custs.get(i).getMoney() + "입니다.");
